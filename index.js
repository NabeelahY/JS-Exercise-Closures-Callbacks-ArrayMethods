...
function processSum(arr, cb) {
  let noSum = arr.reduce((acc, num) => acc + num, 0);
  return cb(noSum);
}
 ...
 * should return 1000.
 */
function processProduct(no1, no2, cb) {
  return cb(no2 * no1);
}
...
function firstNamesAllCaps(arr) {
  let newArr = []
  arr.map(x => {
    newArr.push(x.first_name.toUpperCase())
  })
  return newArr
}
...
function getRunnersByTShirtSize(arr, size) {
  return arr.filter(x => x.shirt_size === size)
}
